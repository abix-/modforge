//! First contact with Corsair Cove (UE 5.4 or 5.5) over the control
//! plane.
//!
...
//!
//! ```text
//! k3sc cargo-lock test -p corsaircove-mod --test research_engine -- --test-threads=1 --nocapture
//! ```
...
fn discovery_counts() {
    ...
    for (op, array_key, count_key) in [
        ...
        if op == "discover_classes" {
            // Words a pirate game is likely to use in its own class
            // names. Guesses until the pak manifest is read; the
            // list printed here is what tells us the real names.
            let words = ["Corsair", "Cove", "Pirate", "Ship", "Crew", "Island"];
