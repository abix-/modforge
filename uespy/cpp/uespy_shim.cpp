// uespy_shim.cpp
//
// Generic UE4SS mod shim shipped in uespy. Compiled by uespy-build
// into every UE4SS Rust mod's cdylib. The mod's metadata + tabs +
// lifecycle hooks all come from extern "C" Rust functions that the
// `uespy::ue4ss_mod!` macro emits. The mod owns no C++ source.
//
// Bridges:
//   uespy_mod_get_name()          -> *const u8 / size  (UTF-8)
//   uespy_mod_get_version()       -> *const u8 / size
//   uespy_mod_get_log_file()      -> *const u8 / size
//   uespy_mod_get_console_title() -> *const u8 / size
//   uespy_mod_get_tab_count()     -> u32
//   uespy_mod_get_tab_name(idx)   -> *const u8 / size
//   uespy_mod_dll_attach(hmod)    -> void
//   uespy_mod_unreal_init()       -> void
//   uespy_mod_shutdown()          -> void
//   uespy_mod_render_tab(idx)     -> void
//
...
extern "C" {
    const std::uint8_t* uespy_mod_get_name(std::size_t* out_len);
    const std::uint8_t* uespy_mod_get_version(std::size_t* out_len);
    const std::uint8_t* uespy_mod_get_log_file(std::size_t* out_len);
    const std::uint8_t* uespy_mod_get_console_title(std::size_t* out_len);
    std::uint32_t       uespy_mod_get_tab_count();
    const std::uint8_t* uespy_mod_get_tab_name(std::uint32_t idx, std::size_t* out_len);
    void                uespy_mod_dll_attach(void* hmod);
    void                uespy_mod_unreal_init();
...
class UespyMod : public RC::CppUserModBase {
  public:
    UespyMod() : CppUserModBase() {
        ModName    = call_str_hook(uespy_mod_get_name);
        ModVersion = call_str_hook(uespy_mod_get_version);

        std::uint32_t count = uespy_mod_get_tab_count();
        if (count > MAX_TABS) count = MAX_TABS;
        for (std::uint32_t i = 0; i < count; ++i) {
            std::size_t n = 0;
            const std::uint8_t* p = uespy_mod_get_tab_name(i, &n);
            if (!p || n == 0) continue;
