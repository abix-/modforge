...
//! every `extern "C"` hook the shipped `uespy_shim.cpp` calls
//! (`uespy_mod_get_name`, `uespy_mod_get_tab_count`,
//! `uespy_mod_render_tab`, `uespy_mod_dll_attach`,
//! `uespy_mod_unreal_init`, `uespy_mod_shutdown`), wires `DllMain`
//! into `uespy::log::set_dll_module`, and initializes the logger
//! on `on_unreal_init`.
//!
//! Game side, in full:
//!
...
//!     tabs: &[
//!         uespy::Tab { name: "Tweaks", render: ows::render_tweaks },
//!         uespy::Tab { name: "Debug",  render: ows::render_debug },
//!     ],
//! };
//!
...
//! ```
//!
//! No `cpp/`, no `build.rs` C++ wiring, no DllMain — uespy ships
//! all of it.
...
macro_rules! ue4ss_mod {
    ($mod_info:ident) => {
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_name(out_len: *mut usize) -> *const u8 {
            unsafe { *out_len = $mod_info.name.len(); }
            $mod_info.name.as_ptr()
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_version(out_len: *mut usize) -> *const u8 {
            unsafe { *out_len = $mod_info.version.len(); }
            $mod_info.version.as_ptr()
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_log_file(out_len: *mut usize) -> *const u8 {
            unsafe { *out_len = $mod_info.log_file.len(); }
            $mod_info.log_file.as_ptr()
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_console_title(out_len: *mut usize) -> *const u8 {
            unsafe { *out_len = $mod_info.console_title.len(); }
            $mod_info.console_title.as_ptr()
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_tab_count() -> u32 {
            $mod_info.tabs.len() as u32
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_get_tab_name(idx: u32, out_len: *mut usize) -> *const u8 {
            let i = idx as usize;
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_dll_attach(hmod: *mut ::std::ffi::c_void) {
            // Forward HMODULE into the logger's DLL-dir resolver.
            $crate::log::set_dll_module(hmod as _);
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_unreal_init() {
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn uespy_mod_shutdown() {
...
/// require it. Today `uespy::log::Config` takes `&'static str`,
