...
| 1 | Ueforge input | [ ] Select the Enhanced Input subsystem by local player instead of the first instance found, so the AI player's second local player can be driven | A test with two live subsystems returns the one owned by the requested local player; `input.rs` no longer takes the first instance. |
...
| 2 | Ueforge navigation | [ ] Generate jump, drop, and crouch links between the navigation mesh edges that have no neighbor, from the character's jump speed, gravity, capsule size, and safe drop height, and store them in a file per level | The file lists links with start, end, and kind; a live test walks one generated jump link with the AI player's character in Abiotic Factor. |
| 2 | modforge route | [ ] Record the human player's own jumps as links: where the player left the navigation mesh and where they rejoined it | After one play session the links file holds recorded links with start and end; a unit test rejects a link whose ends lie on the same mesh polygon. |
| 3 | modforge route | [ ] Pick the next waypoint by need: each candidate advertises how much it satisfies a need, scored by urgency and path cost | A unit test with hunger high and two reachable containers selects the food container; with hunger low it selects the cheaper path. |
