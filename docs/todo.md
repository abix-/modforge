...
- [/] **I-5: Test harness.** First smoke `horsey-mod/tests/input_smoke.rs` shipped 2026-05-16. Covers L1 cursor round-trip + L1 keyboard + L2 PostMessage no-op. Earlier run hit a pre-existing horsey-side resolver regression (NO_TIRE_TOGGLE worker write off by `0x40_00_00_00`). User reports other-Claude is done; smoke re-running 2026-05-16. Pending live result; doc update follows.
