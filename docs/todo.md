...
| 1 | Ueforge symbols | [x] Add a PDB reader (the `pdb` crate) that, given a PDB path, returns a function's image-relative address and a struct's field layout | `ueforge::symbols::{functions, layout}`, commit 651801ae. `research_symbols` on Abiotic Factor's PDB: APlayerController::InputKey(const FInputKeyParams&) at 0x3AB2890, UPlayerInput::InputKey at 0x3DA2190, FInputKeyParams 72 bytes (layout in abioticfactor-mod/docs/character.md). Awaiting changelog entry before removal. |
