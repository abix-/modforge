//! rust_print: lift a function from Horsey.exe to Rust-shaped pseudocode.
//!
//! Pipeline:
...
//!   5. Pass: recognize x86 flag-set patterns, rewrite flag reads as boolean exprs
//!   6. Pass: recognize x86 call pattern (push retaddr; branch target) -> Stmt::Call
//!   7. Pass: recognize x86 ret pattern (pop into temp; branch temp)  -> Stmt::Return
...
fn print_rust(ir: &RustIr, names: &HashMap<u64, String>) {
    let fn_name = pretty_fn_name(ir.addr, names);
    println!("// Decompiled from Horsey.exe @ 0x{:x}", ir.addr);
    println!("// Pipeline: Falcon lift -> dce -> fold -> flag-recog -> call/ret-recov -> Rust print");
    println!("// blocks: {}", ir.blocks.len());
    println!();
    println!("#[allow(non_snake_case, unused_assignments, unused_variables, unused_mut, dead_code)]");
    println!("pub unsafe fn {fn_name}() {{");
    ...
    for block in &ir.blocks {
        ...
        println!();
        println!("    // ---- bb{} @ {label} ----", block.index);
        for s in &block.stmts {
            println!("    {}", print_stmt(s, names));
        }
        match block.succs.as_slice() {
            [] => println!("    // <no successors>"),
            [(None, t)] => println!("    /* fallthrough -> bb{t} */"),
            [(Some(a), ta), (Some(b), tb)] if conds_are_inverse(a, b) => {
                println!("    if {} {{", print_expr_bool(a));
                println!("        /* -> bb{ta} */");
                println!("    }} else {{");
                println!("        /* -> bb{tb} */");
                println!("    }}");
            ...
            multi => {
                for (cond, t) in multi {
                    match cond {
                        Some(c) => println!("    if {} {{ /* -> bb{t} */ }}", print_expr_bool(c)),
                        None => println!("    /* -> bb{t} */"),
                    }
                }
            }
