<#
.SYNOPSIS
Rebuild, redeploy, and restart Alchemy Factory with the current mod.

.DESCRIPTION
The one path for getting the working tree into the live game.
Steps, in order:
  1. Build alchemyfactory-mod (Rust release, via k3sc cargo-lock).
  2. Close the game if it is running and wait for it to exit.
  3. Ensure UE4SS is installed (copy from Sintopia if missing; that
     install carries the minimal hooks settings file).
  4. Deploy: copy the built DLL to the UE4SS mod directory,
     removing any stale main-new.dll.
  5. Launch the game through Steam.
  6. Wait until the control plane answers on port 31290.
...
.EXAMPLE
pwsh -NoProfile -File alchemyfactory-mod/scripts/restart.ps1

.EXAMPLE
pwsh -NoProfile -File alchemyfactory-mod/scripts/restart.ps1 -SkipBuild
#>
[CmdletBinding()]
param (
    [switch]$SkipBuild,
...
$GameDir = "C:\Games\Steam\steamapps\common\Alchemy Factory"
$BinDir = Join-Path $GameDir "AlchemyFactory\Binaries\Win64"
$UE4SSDir = Join-Path $BinDir "ue4ss"
$ModDir = Join-Path $UE4SSDir "Mods\AlchemyFactoryMod\dlls"
$BuildDll = Join-Path $Repo "target\x86_64-pc-windows-msvc\release\alchemyfactory_mod.dll"
$TargetDll = Join-Path $ModDir "main.dll"
$StaleDll = Join-Path $ModDir "main-new.dll"
$AppId = "3669570"
$Port = 31290
$ProcessNames = @("AlchemyFactory-Win64-Shipping", "AlchemyFactory")

# Reference UE4SS installation to copy from. Sintopia's install is
# the MISERY UE4SS.dll plus the minimal settings file (every hook
# off except EngineTick and ProcessEvent, GUI console off).
$RefUE4SS = "C:\Games\Steam\steamapps\common\Sintopia\Sintopia\Binaries\Win64"

# --- step 1: build ---
if (-not $SkipBuild) {
    Write-Host "[build] alchemyfactory-mod (Rust release)" -ForegroundColor Cyan
    Push-Location $Repo
    try {
        k3sc cargo-lock build --release -p alchemyfactory-mod
...
# Identity check: prove this DLL belongs to alchemyfactory-mod
$marker = "alchemyfactory_mod.log"
$bytes = [System.IO.File]::ReadAllBytes($BuildDll)
$ascii = [System.Text.Encoding]::ASCII.GetString($bytes)
if ($ascii -notmatch [regex]::Escape($marker)) {
    throw "[build] IDENTITY FAILED: $BuildDll does not contain '$marker'; this is not the alchemy factory mod"
}
...
# Ensure mods.txt enables AlchemyFactoryMod
$modsTxt = Join-Path $UE4SSDir "Mods\mods.txt"
$modEntry = "AlchemyFactoryMod : 1"
if (Test-Path $modsTxt) {
    $content = Get-Content $modsTxt -Raw
    if ($content -notmatch "AlchemyFactoryMod") {
        Add-Content $modsTxt "`n$modEntry"
        Write-Host "[deploy] added AlchemyFactoryMod to mods.txt" -ForegroundColor Gray
    }
}
else {
    Set-Content $modsTxt $modEntry
    Write-Host "[deploy] created mods.txt with AlchemyFactoryMod" -ForegroundColor Gray
}
...
if (-not $cpReady) {
    Write-Warning "control plane not answering after 180s; check UE4SS.log"
    exit 1
}

Write-Host "[done] Alchemy Factory running with mod loaded" -ForegroundColor Green
exit 0
