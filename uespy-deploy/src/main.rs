//! `uespy-deploy` — framework-level build + install + package
//! CLI for any uespy-based UE4SS Rust mod in the workspace.
//!
//! Each mod opts in by adding a `[package.metadata.uespy]`
//! block to its `Cargo.toml`. Run from anywhere in the
//! workspace:
//!
//! ```text
//! cargo run -p uespy-deploy --release -- install -p tweaks
//! cargo run -p uespy-deploy --release -- install -p better-backpack
//! ```
//!
...
#[derive(Parser)]
#[command(version, about = "uespy mod deploy CLI", long_about = None)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn read_mod_cfg(package: &str) -> Result<ModCfg> {
    ...
    let raw = pkg
        .metadata
        .as_ref()
        .and_then(|v| v.get("uespy"))
        .ok_or_else(|| {
            anyhow!(
                "package '{package}' is missing its [package.metadata.uespy] block.\n\n\
    ...
    let u: UespyMetadata =
        serde_json::from_value(raw.clone()).context("parsing [package.metadata.uespy]")?;
    ...
    let game_name_regex = Regex::new(&u.game_name_regex)
        .context("compiling [package.metadata.uespy].game_name_regex")?;
