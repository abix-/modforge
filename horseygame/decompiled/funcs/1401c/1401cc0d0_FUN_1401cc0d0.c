// Address: 0x1401cc0d0
// Ghidra name: FUN_1401cc0d0
// ============ 0x1401cc0d0 FUN_1401cc0d0 (size=362) ============


void FUN_1401cc0d0(longlong param_1,longlong param_2)



{

  uint uVar1;

  longlong lVar2;

  undefined4 uVar3;

  longlong lVar4;

  uint uVar5;

  ulonglong uVar6;

  

  lVar4 = FUN_1401c9f40(param_2);

  if (lVar4 != 0) {

    if (*(longlong *)(lVar4 + 8) != param_1) {

      FUN_14012e850("Window not claimed by this device");

      return;

    }

    if (1 < *(int *)(lVar4 + 0x10)) {

      *(int *)(lVar4 + 0x10) = *(int *)(lVar4 + 0x10) + -1;

      return;

    }

    FUN_1401cd410(param_1);

    uVar6 = 0;

    if (*(longlong *)(lVar4 + 0x128) != 0) {

      FUN_1401cbf30(param_1);

      *(undefined8 *)(lVar4 + 0x128) = 0;

    }

    if (*(longlong *)(lVar4 + 0x130) != 0) {

      FUN_1401cbf30(param_1);

      *(undefined8 *)(lVar4 + 0x130) = 0;

    }

    if (*(longlong *)(lVar4 + 0x138) != 0) {

      FUN_1401cbf30(param_1);

      *(undefined8 *)(lVar4 + 0x138) = 0;

    }

    FUN_1401c9cd0(param_1,lVar4);

    FUN_140179b40(*(undefined8 *)(param_1 + 0x218));

    uVar1 = *(uint *)(param_1 + 0x148);

    if (uVar1 != 0) {

      lVar2 = *(longlong *)(param_1 + 0x140);

      do {

        if (**(longlong **)(lVar2 + uVar6 * 8) == param_2) {

          *(undefined8 *)(lVar2 + uVar6 * 8) = *(undefined8 *)(lVar2 + (ulonglong)(uVar1 - 1) * 8);

          *(int *)(param_1 + 0x148) = *(int *)(param_1 + 0x148) + -1;

          break;

        }

        uVar5 = (int)uVar6 + 1;

        uVar6 = (ulonglong)uVar5;

      } while (uVar5 < uVar1);

    }

    FUN_140179b60(*(undefined8 *)(param_1 + 0x218));

    FUN_1401841e0(lVar4);

    uVar3 = FUN_14016ea50(param_2);

    FUN_1401745e0(uVar3,"SDL.internal.gpu.d3d12.data");

    FUN_1401cda70(1,FUN_1401ca640,param_2);

  }

  return;

}




