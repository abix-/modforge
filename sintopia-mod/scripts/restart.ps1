<#
.SYNOPSIS
Rebuild, redeploy, and restart Sintopia with the current mod.

.DESCRIPTION
The one path for getting the working tree into the live game.
Steps, in order:
  1. Build sintopia-mod (Rust release, via k3sc cargo-lock).
  ...
  6. Wait until the control plane answers on port 31270.
...
.EXAMPLE
pwsh -NoProfile -File sintopia-mod/scripts/restart.ps1

.EXAMPLE
pwsh -NoProfile -File sintopia-mod/scripts/restart.ps1 -SkipBuild
#>
[CmdletBinding()]
param (
    [switch]$SkipBuild,
...
$GameDir = "C:\Games\Steam\steamapps\common\Sintopia"
$BinDir = Join-Path $GameDir "Sintopia\Binaries\Win64"
$UE4SSDir = Join-Path $BinDir "ue4ss"
$ModDir = Join-Path $UE4SSDir "Mods\SintopiaMod\dlls"
$BuildDll = Join-Path $Repo "target\x86_64-pc-windows-msvc\release\sintopia_mod.dll"
$TargetDll = Join-Path $ModDir "main.dll"
$StaleDll = Join-Path $ModDir "main-new.dll"
$AppId = "2213700"
$Port = 31270
$ProcessNames = @("Sintopia-Win64-Shipping", "Sintopia")
...
# --- step 1: build ---
if (-not $SkipBuild) {
    Write-Host "[build] sintopia-mod (Rust release)" -ForegroundColor Cyan
    Push-Location $Repo
    try {
        k3sc cargo-lock build --release -p sintopia-mod
...
# Identity check: prove this DLL belongs to sintopia-mod
$marker = "sintopia_mod.log"
$bytes = [System.IO.File]::ReadAllBytes($BuildDll)
$ascii = [System.Text.Encoding]::ASCII.GetString($bytes)
if ($ascii -notmatch [regex]::Escape($marker)) {
    throw "[build] IDENTITY FAILED: $BuildDll does not contain '$marker'; this is not the sintopia mod"
}
...
# Ensure mods.txt enables SintopiaMod
$modsTxt = Join-Path $UE4SSDir "Mods\mods.txt"
$modEntry = "SintopiaMod : 1"
if (Test-Path $modsTxt) {
    $content = Get-Content $modsTxt -Raw
    if ($content -notmatch "SintopiaMod") {
        Add-Content $modsTxt "`n$modEntry"
        Write-Host "[deploy] added SintopiaMod to mods.txt" -ForegroundColor Gray
    }
}
else {
    Set-Content $modsTxt $modEntry
    Write-Host "[deploy] created mods.txt with SintopiaMod" -ForegroundColor Gray
}
...
Write-Host "[done] Sintopia running with mod loaded" -ForegroundColor Green
exit 0
