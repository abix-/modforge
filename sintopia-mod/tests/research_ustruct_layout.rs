//! Where do PropertiesSize and ChildProperties live on UStruct in
//! this game's UE 5.2.1?
//!
//! discover_class_detail on native Sintopia classes returned
//! fields=[] and properties_size=2971203040 (a pointer's low
//! half), so ueforge's UStruct offsets (measured on UE 5.4) are
//! wrong here. This test measures them from raw bytes:
//!
...
//!
//! ```text
//! k3sc cargo-lock test -p sintopia-mod --test research_ustruct_layout -- --test-threads=1 --nocapture
//! ```
...
fn properties_size_slot() {
    ...
    println!("ueforge assumes +0xb0; Object reads {:#x} there", i32_at(&bytes["Object"], 0xB0));
}
...
/// ueforge assumes ElementSize +0x34 and Offset_Internal +0x4C.
#[test]
fn fproperty_slots() {
    let api = api();
    if ping_or_skip(&api).is_none() {
